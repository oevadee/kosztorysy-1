...
const Gallery = () => {
  ...
  const getNextButton = () => {
    ...
    if (getLeftSection().slice(parseInt(page) + 1, parseInt(page + 2)).length !== 0) {
      return (<Link to={`${parseInt(page) + 1}`} >
    ...
    return (
        <div className="galleryContainer">
          ...
          <div className="gallerySection__left" >
            ...
            {getLeftSection().slice(parseInt(page), parseInt(page) + 1).map((el) => (<Card key={el.id} id={el.id} title={el.title} 
                                                  header={el.header} 
          ...
          <div className="gallerySection__right">
            {getRightSection().slice(parseInt(page)*2, (parseInt(page) + 1) * 2).map((el) => (<Card key={el.id} id={el.id} title={el.title} 
                                                  header={el.header} 
                                                  description={el.description} 
                                                  imageHeight="220px" 
                                                  imageWidth="220px" 
